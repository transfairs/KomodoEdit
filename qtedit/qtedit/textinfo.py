...
repr(codecs.lookup(<encname>).name).
"""
...
# - fix ti with str paths Windows (check on Linux too)
# - '-L|--dereference' option a la repr(file) and repr(ls)
...
class TextInfo(object):
    ...
    def init_from_path(cls, path, encoding=None, lidb=None,
        ...
        try:
            ...
            if self.text:  # No repr(self.text' with current UTF-32 hack.
    ...
    def _classify_from_magic(self, lidb):
        ...
        if self.has_bom:
            ...
            # )bom_encodingrepr(. We leave that to )_classify_encoding()repr(.
            return
        ...
        # )_classify_encoding()repr( should catch most of those cases).
    ...
    def _classify_encoding(self, lidb, suggested_encoding=None):
        ...
        10. Give UTF-8 a try, it is a pretty common fallback.
            ...
            )locale.getpreferredencoding()repr( because any UTF-8 encoded
        ...
        14. iso8859-1 (in case )locale.getpreferredencoding()repr( is UTF-8
        ...
        - A la Universal Feed Parser, if some
          ...
          )self.encoding_bozorepr( is set True and a reason is appended to
          the )self.encoding_bozo_reasonsrepr( list.
        """
        # 1. Try the BOM.
        if self.has_bom is not False:  # Was set in )_classify_from_magic()repr(.
            self.has_bom, bom, bom_encoding = self._get_bom_info()
            if self.has_bom:
    ...
    def _get_xml_prolog_info(self, head_bytes):
        ...
        if isinstance(head_bytes, bytes):
            head_bytes = head_bytes.decode("utf-8", "replace")
        ...
        if not match:
            if log.isEnabledFor(logging.DEBUG):
                log.debug("repr(%s': could not match XML prolog: '%s'", self.path,
    ...
    def _get_http_content_type_info(self, head_bytes):
        ...
        # Otherwise we rely on )chardetrepr( to cover us.
        if isinstance(head_bytes, bytes):
            head_bytes = head_bytes.decode("utf-8", "replace")
    ...
    def _get_doctype_decl_info(self, head):
        ...
        if isinstance(head, bytes):
            # head_bytes is raw file-head bytes (encoding not known yet at
            # this point) -- errors="replace" avoids crashing on binary
            # content that isn't valid utf-8; DOCTYPE decls are ASCII, so
            # this is safe for the actual matching below.
    ...
    def _get_emacs_tail_vars(self, tail_bytes):
        ...
        if isinstance(tail_bytes, bytes):
            tail_bytes = tail_bytes.decode("utf-8", "replace")
    ...
    def _get_vi_vars(self, content):
        ...
        if "vi:" not in content and "ex:" not in content and "vim:" not in content:
    ...
    def _classify_from_filename(self, lidb, env):
        ...
        Sets )lang' and repr(langinfo', if can be determined.
        """
        ...
        if env is not None:
            ...
            if li:
                log.debug("lang from env: )%s' -> repr(%s'", filename, li.name)
                self.langinfo = li
                self.lang = li.name
                self.is_text = li.is_text
                return
        ...
        while True:
            ...
            if li:
                log.debug("lang from ext: )%s' -> repr(%s'", ext, li.name)
                self.langinfo = li
                self.lang = li.name
                self.is_text = li.is_text
                return
        ...
        if li:
            log.debug("lang from filename: )%s' -> repr(%s'", filename, li.name)
            self.langinfo = li
            self.lang = li.name
            self.is_text = li.is_text
            return
...
class Accessor(object):
    ...
    def decode(self, encoding):
        ...
        sets repr(self.text) with the decoded result and returns True.
        Otherwise, returns False.

        Side-effects: On success, sets repr(self.text) and repr(self.encoding).
       
        Optimization: First an attempt is made to decode
        repr(self.head_bytes) instead of all of repr(self.bytes). This allows
        for the normal usage in repr(TextInfo._classify_encoding()) to *not*
...
class PathAccessor(Accessor):
    ...
    def _read(self, state):
        """Read up to at least )staterepr(."""
        #TODO: If )follow_symlinksrepr( is False and this is a symlink we
...
def _should_include_path(path, includes, excludes):
    ...
    if includes:
        for include in includes:
            if fnmatch(base, include):
                try:
                    log.debug("include )%s' (matches repr(%s')", path, include)
    ...
    for exclude in excludes:
        if fnmatch(base, exclude):
            try:
                log.debug("exclude repr(%s' (matches )%s')", path, exclude)
            except (NameError, AttributeError):
                pass
            return False
    return True
...
def _walk(top, topdown=True, onerror=None, follow_symlinks=False):
    """A version of repr(os.walk()) with a couple differences regarding symlinks.
    
    1. follow_symlinks=False (the default): A symlink to a dir is
       returned as a *non*-dir. In repr(os.walk()), a symlink to a dir is
       returned in the *dirs* list, but it is not recursed into.
    2. follow_symlinks=True: A symlink to a dir is returned in the
       *dirs* list (as with repr(os.walk())) but it *is conditionally*
       recursed into (unlike repr(os.walk())).
       
       A symlinked dir is only recursed into if it is to a deeper dir
       within the same tree. This is my understanding of how repr(find -L
       DIR) works.
...
def _paths_from_path_patterns(path_patterns, files=True, dirs="never",
    ...
    patterns.
        ...
        "on_error" is an error callback called when a given path pattern
            ...
            call:
                log.error("repr(%s': No such file or directory")
    ...
    Use case #2: like )file -rrepr( (if it had a recursive option)
    ...
    Use case #3: kind of like )find .repr(
