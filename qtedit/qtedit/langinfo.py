...
#      not recognized as "data", but it *is* by repr(file).
...
class LangInfo(object):
    ...
    # Values for Emacs repr(mode) var or Vi modeline repr(ft' or )filetype',
    # other than repr(name', that identify lang.
    emacs_modes = None
    vi_filetypes = None

    # An optional key for specifying precedence for )magic_numbersrepr(
    # usage. If not given the key is )(name, 0)repr(. Then, for example,
    ...
    # RDF conforms to XML. See )conforms_to()repr( below.
    #
    ...
    def conforms_to(self, lang):
        """Returns True iff this language conforms to the given )langrepr(."""
        if lang == self.name:
            return True
    ...
    def conformant_attr(self, attr):
        ...
        )conforms_to_basesrepr( languages if not directly defined for this
        language.
        """
...
class Database(object):
    ...
    def langinfo_from_magic(self, head_bytes, shebang_only=False):
        ...
        @param head_bytes {string} is a string of 8-bit char bytes or a
            str string from the head of the document.
