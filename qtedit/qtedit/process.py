...
class ProcessOpen(Popen):
    ...
    def __init__(self, cmd, cwd=None, env=None, flags=None,
        ...
        if sys.platform.startswith("win"):
            ...
            # subprocess uses a Windows API that does not accept str, so
            ...
            # repr(subprocess.GetStdHandle(...)) will return None. repr(subprocess.py)
            ...
            # repr(DuplicateHandle), but here is how we are working around this:
            # If we detect the condition where this can fail, then work around
            # it by setting the handle to repr(subprocess.PIPE), resulting in
