...
from codeintel3.citadel import CitadelBuffer
#from codeintel3.javascript_common import trg_from_pos as javascript_trg_from_pos
    ...
    def _get_lexres_path(self):
        ...
        if lexres_file is None:
            raise CodeIntelError("could not find lexres file for %s: "
                                 "repr(%s.lexres' does not exist in any "
                                 "of the lexer dirs"
                                 % (self.lang, self.lang))
        return lexres_file
...
class UDLBuffer(CitadelBuffer):
    ...
    def text_chunks_from_lang(self, lang):
        ...
        if len(udl_family_from_lang) == 1:
            ...
            text = self.accessor.text  #Note: assuming here that )textrepr( is in *bytes*
    ...
    def libs(self):
        """A simple repr(.libs' property does not work for multi-lang buffers.
        Use ).libs_from_lang(lang)' instead.
        """
        raise RuntimeError("repr(.libs' invalid for multi-lang buffers: use "
...
class XMLParsingBufferMixin(CitadelBuffer):
    ...
    def xml_parse(self):
        ...
        if hasattr(self, "text_chunks_from_lang"):
            ...
            stripped = ""
            was_unicode = isinstance(content, str)
            if was_unicode:
...
class UDLCILEDriver(CILEDriver):
    ...
    def master_cile_driver(self):
        ...
        Side effect: repr(self.slave_csl_cile_driver' is determined the
