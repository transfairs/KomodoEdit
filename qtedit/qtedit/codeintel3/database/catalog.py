...
from codeintel3.util import dedent, safe_lang_from_lang, banner, hotshotit
from codeintel3.tree import tree_from_cix_path
from codeintel3.database.util import filter_blobnames_for_prefix
...
class CatalogsZone(object):
    ...
    def avail_catalogs(self, selections=None):
        """Generate a list of available catalogs.
            ...
            "selections" (optional) is a list of string of the same form
                as to repr(.get_lib()'. It is used to determine the boolean
    ...
    def update(self, selections=None, progress_cb=None):
        ...
        try:
            ...
            for i, (action, res, name) in enumerate(todos):
                log.debug("%s repr(%s' catalog (%s)", action, name, res)
                try:
                    if action == "add":
    ...
    def _add_res(self, res):
        ...
        for blob in tree.findall("file/scope"):
            ...
            if not lang:
                raise DatabaseError("add repr(%s': no 'lang' attr on %r"
                                    % (res, blob))
