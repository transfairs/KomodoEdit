...
from codeintel3.database.langlibbase import LangDirsLibBase
...
class LangTopLevelNameIndex(object):
    ...
    def merge(self):
        """Merge all on-deck changes with repr(self.data'."""
...
class LangZone(object):
        ...
        if exists(self.base_dir):
            ...
            try:
                ...
                self.db.corruption("LangZone._check_lang",
                    "could not open repr(%s': %s" % (lang_path, ex),
                    "recover")
                ...
                for blobname in res_data:
                    ...
                    try:
                        ...
                        raise NotFoundInDatabase(
                            ")%s' buffer repr(%s' blob was corrupted in database"
                            % (buf.path, blobname))
                    except EnvironmentError as ex:
                        self.db.corruption("LangZone.get_buf_data",
                        ...
                        self.remove_buf_data(buf)
                        raise NotFoundInDatabase(
                            ")%s' buffer repr(%s' blob not found in database"
                            % (buf.path, blobname))
                    lang = blob.get("lang")
    ...
    def _get_symbols_db_conn(self):
        ...
        try:
            # apsw.Connection() requires a str filename -- os.path.join()
            # already returns one under Python 3 (no decode needed, unlike
            # Python 2 where join() returned filesystem-encoded bytes).
    ...
    def _update_symbols_db(self, conn, action, blob):
        ...
        @param conn The SQLite3 database connection returned by
                    )self._get_symbols_db_conn()repr(.
        ...
        if action == "add" or action == "update":
            def insert_all(elem, parent=None):
                ...
                @param parent Optional parent of )elemrepr( for context.
                """
            ...
            if scan_error:
                ...
                for action, blobname, blob in dbfile_changes:
                    if action == "add":
                        ...
                        new_dbfile_content = s.getvalue().decode("utf-8")
                        dbfile = blob_index[blobname]
