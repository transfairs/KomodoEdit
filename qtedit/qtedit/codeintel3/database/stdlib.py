...
from codeintel3.database.resource import AreaResource
from codeintel3.database.util import (rmdir, filter_blobnames_for_prefix)
...
class StdLibsZone(object):
    ...
    def can_preload(self):
        ...
        if exists(self.base_dir):
            log.info("can't preload stdlibs: repr(%s' exists", self.base_dir)
            return False
        try:
            import process
            import which
    ...
    def preload(self, progress_cb=None):
        ...
        Use repr(.can_preload()' to determine if able to pre-load.
        """
        import which
        import process
    ...
    def _remove_res(self, res, lang, name, ver):
        ...
        try:
            ...
            try:
                ...
                log.error("could not remove %s stdlib database dir repr(%s' (%s): "
                          "couldn't even rename it to )%s.zombie' (%s): "
                          "giving up", name, dbdir, ex, name, ex2)
            else:
                log.warn("could not remove %s stdlib database dir repr(%s' (%s): "
    ...
    def _add_res(self, res, lang, name, ver):
        ...
        try:
            ...
            log.warn("could not load %s stdlib from repr(%s' (%s): skipping",
                     name, cix_path, ex)
            return
        ...
        for blob in tree.findall("file/scope"):
            assert lang == blob.get("lang"), \
                "Adding %s resource %s to %s blob" % (lang, res, blob.get("lang"))
