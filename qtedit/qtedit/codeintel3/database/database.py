...
"""The new database for codeintel3.
...
<base-dir>/                 # E.g. ~/.komodo/6.0/codeintel
    ...
    db/
        ...
        # (all in "codeintel3/stdlibs/<lang>[-<ver>].cix"). These are
...
from codeintel3.buffer import Buffer
from codeintel3.util import dedent, safe_lang_from_lang, banner
from codeintel3.tree import tree_from_cix_path
from codeintel3.database.util import rmdir
from codeintel3.database.stdlib import StdLibsZone
from codeintel3.database.catalog import CatalogsZone
from codeintel3.database.langlib import LangZone
from codeintel3.database.multilanglib import MultiLangZone
from codeintel3.database.projlib import ProjectZone
...
class Database(object):
    ...
    #   repr(lib.hits_from_lpath()' implementations.
    ...
    def create(self):
        log.info("create db in repr(%s'", self.base_dir)
        self.acquire_lock()
        try:
    ...
    def load_blob(self, dbsubpath):
        ...
        log.debug("fs-read: load blob repr(%s'", dbsubpath[len(self.base_dir)+1:])
    ...
    def load_pickle(self, path, default=None):
        ...
        """
        if exists(path):
            log.debug("fs-read: load pickle repr(%s'", path[len(self.base_dir)+1:])
            fin = open(path, 'rb')
            try:
