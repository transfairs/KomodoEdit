...
# Python 3's own `ast` module replaces Python 2's `compiler`/`compiler.ast`/
# `compiler.visitor`/`parser` packages used here originally -- all four are
# gone from the stdlib entirely (no PyPI backport either). This isn't a
# syntax-only port: `ast`'s node types/shapes differ structurally from
# `compiler.ast`'s (e.g. one BinOp node with an `.op` operator instance
# instead of a separate node class per operator; AssName/AssAttr/AssTuple
# folded into Name/Attribute/Tuple with a Store() ctx; CallFunc's separate
# star_args/dstar_args folded into `args`/`keywords`; etc.) -- ported node
# type checks in AST2CIXVisitor below are one-for-one, verified against
# real `ast.dump()` output rather than guessed. `ast.parse()` handles
# genuine Python 3 syntax natively, which also means the old
# `_convert3to2()` regex-based Python-3-to-2 downgrade hack (needed only
# because `compiler.parse()` couldn't read Python 3 syntax at all) is
# removed entirely, not just ported -- see scan_et() below.
import ast

from codeintel3.common import CILEError
from codeintel3 import util
...
def getAttrStr(attrs):
    ...
    """
    # Dead code (no caller in this file, kept only because the original
    # had it) -- the Python-2-era `.encode("utf-8")` branch here would
    # have handed quoteattr() a bytes object under Python 3, which it
    # doesn't accept (it wants str); fixed while porting even though
    # unreachable, so it isn't a landmine for a future caller.
...
def _unistr(x):
    # Python 2 had this branch on `unicode` vs `str` (bytes); Python 3
    # strings are already unicode, so the `.decode('utf8')` branch that
    # used to handle bytes-vs-text is dead code now, not ported.
    if isinstance(x, str):
        return x
...
class AST2CIXVisitor(ast.NodeVisitor):
    ...
    Subclasses ast.NodeVisitor instead of hand-rolling dispatch (as the
    original did against compiler.visitor.ASTVisitor): only the ~10
    "interesting" node types below get a visit_X override; every other
    node type (If/While/AugAssign/expression statements/...) falls
    through to NodeVisitor's own generic_visit(), which recurses into all
    child nodes automatically -- exactly the same "no handler -> just
    recurse into children" behavior compiler.walk() provided, so nested
    Function/Class/Assign/Import/etc. inside an `if` block etc. still get
    found without this class needing to special-case every statement type.
    """
    DEBUG = 0
    ...
    def _visitBody(self, stmts):
        """Visit a list of statement nodes -- Python 3's `ast` uses plain
        lists for Module.body/FunctionDef.body/ClassDef.body/etc. (no
        wrapping Stmt node the way compiler.ast did), so this replaces the
        old `self.visit(node.code)`/`self.visit(node.node)` calls."""
        for stmt in stmts:
            self.visit(stmt)
    ...
    def visit_ClassDef(self, node):
        ...
        if node.body:
            ...
            if getattr(lastNode, "lineno", None):
    ...
    def visit_FunctionDef(self, node):
        ...
        if node.decorator_list:
            for deco in node.decorator_list:
                # A decorator is any expression -- Name (@foo), Attribute
                # (@foo.bar), or Call (@foo(...)) are the common ones. Old
                # code read `.name`/`.attrname` off compiler.ast's Name/
                # Getattr nodes directly; ast's equivalents are `.id`/`.attr`.
                ...
                if deco_name == 'property':
                    prop_mode = 'getter'
                elif deco_attr in ('getter', 'setter', 'deleter'):
                    prop_mode = deco_attr
        ...
        # Handle arguments. `node.args` is an `ast.arguments` object:
        # posonlyargs + args (regular, in order), vararg (*args, or None),
        # kwonlyargs (keyword-only, after *args or bare *), kw_defaults
        # (one per kwonlyarg, None if no default), kwarg (**kwargs, or
        # None), defaults (trailing defaults for posonlyargs+args, same
        # "last N args" convention the old argnames/defaults pair used).
        # Positional-only args (Python 3.8+ `def f(a, /, b)`) are treated
        # like regular args here -- codeintel has no separate concept for
        # them and the call-signature text looks the same either way.
        a = node.args
        posargs = list(a.posonlyargs) + list(a.args)
        argnames = [arg.arg for arg in posargs]
        defaults = list(a.defaults)  # aligned to the *end* of posargs, like old code
        defaultArgsBaseIndex = len(argnames) - len(defaults)
        ...
        for i, argName in enumerate(argnames):
            ...
            if i >= defaultArgsBaseIndex:
                defaultNode = defaults[i-defaultArgsBaseIndex]
                try:
            ...
            argument["declaration"] = argument.get("declaration", argument)
            namespace["arguments"].append(argument)
            namespace["symbols"][argument["name"]] = argument

        if a.vararg:
            argName = a.vararg.arg
            argument = {"name": argName, "nspath": nspath+(argName,),
                        "doc": None, "types": {}, "line": node.lineno,
                        "symbols": {}, "attributes": "varargs"}
            argument["declaration"] = argument
            sigArgs.append("*"+argName)
            namespace["arguments"].append(argument)
            namespace["symbols"][argName] = argument

        for i, kwarg in enumerate(a.kwonlyargs):
            argName = kwarg.arg
            argument = {"name": argName, "nspath": nspath+(argName,),
                        "doc": None, "types": {}, "line": node.lineno,
                        "symbols": {}}
            default_node = a.kw_defaults[i] if i < len(a.kw_defaults) else None
            if default_node is not None:
                try:
                    argument["default"] = self._getExprRepr(default_node)
                except PythonCILEError as ex:
                    raise PythonCILEError("unexpected default argument node "
                                          "type for Function '%s': %s"
                                          % (node.name, ex))
    ...
    # `async def` functions get identical CIX handling to regular ones --
    # codeintel has no separate concept for coroutines.
    ...
    def visit_Import(self, node):
        ...
        for alias in node.names:
            ...
            if alias.asname: import_["alias"] = alias.asname
    ...
    def _visitSimpleAssign(self, lhsNode, rhsNode, line):
        ...
        Note: compiler.ast had dedicated AssName/AssAttr/AssTuple/AssList
        node types for assignment targets; ast (Python 3) reuses the plain
        Name/Attribute/Tuple/List expression types for both loading and
        storing, distinguished by a `.ctx` of Store() vs Load() -- since
        this is only ever called with an assignment-target node, a bare
        isinstance(lhsNode, ast.Name) etc. check is equivalent to the old
        isinstance(lhsNode, ast.AssName) check.
        """
    ...
    def visit_Assign(self, node):
        ...
        if isinstance(lhsNode, (ast.Name, ast.Attribute)):
            ...
            # E.g.:  bar[1:2] = "foo"  or  bar[1] = "foo"
            # We don't bother with these: too hard (old code split this
            # into a separate ast.Slice case, but Python 3 always
            # represents subscript-assignment as Subscript regardless of
            # whether the index is a slice or a plain expression).
            log.info("visitAssign:: skipping subscript/slice - too hard")
            pass
        else:
    ...
    def visit_With(self, node):
        log.info("visitWith:%d: %r", node.lineno,
                 self.lines and self.lines[node.lineno-1])
        # Python 3's `with` can have multiple context managers
        # (`with a() as x, b() as y:`) via node.items -- compiler.ast's
        # With only ever had one (vars, expr) pair; loop over all of them
        # here instead.
        for item in node.items:
            lhsNode = item.optional_vars
            rhsNode = item.context_expr
            if lhsNode is None:
                continue  # `with x():` with no `as` clause -- nothing to assign
            self._handleUnknownAssignment(lhsNode, node.lineno)
            if isinstance(lhsNode, ast.Name):
                # E.g.:
                #   with x() as foo:
                self._visitSimpleAssign(lhsNode, rhsNode, node.lineno)
                ...
                #   with x() as (foo, bar):
                #   with x() as [foo, bar]:
                # If the expression is a sequence with the same number of
                # elements, then we update each assigned-to variable.
                # Otherwise, bail.
                if isinstance(rhsNode, (ast.Tuple, ast.List)):
                    if len(lhsNode.elts) == len(rhsNode.elts):
                        for i in range(len(lhsNode.elts)):
    ...
    def visit_Try(self, node):
        ...
        for handler in node.handlers:
            # Python 3's ExceptHandler.name is a plain string (or None) --
            # `except Foo as e:` no longer represents `e` as a separate
            # AssName node the way compiler.ast did, so this registers it
            # as a variable directly instead of routing through
            # _handleUnknownAssignment (which expects an ast node).
            if handler.name:
                self._visitSimpleAssign(
                    ast.Name(id=handler.name, ctx=ast.Store()),
                    None, handler.lineno or node.lineno)
            self._visitBody(handler.body)
        if node.orelse:
            self._visitBody(node.orelse)
        # Python 3 merges what used to be a separate TryFinally wrapper
        # (which this file never had a visitTryFinally for, so finally
        # blocks were walked only via compiler.walk's default generic
        # recursion) into this same Try node's `finalbody` -- visited
        # explicitly here so nothing regresses.
        if node.finalbody:
            self._visitBody(node.finalbody)
        ...
        if isinstance(expr, ast.Name):
            ...
            if (decl #and "symbols" in decl #XXX this "and"-part necessary?
                ...
                citdl = "__builtins__.%s.%s"\
                        % ((type(expr.value.value).__name__), expr.attr)
                return (None, citdl)
    ...
    # compiler.ast had a separate node CLASS per binary/unary/bool operator
    # (Add, Sub, Bitand, Or, UnarySub, ...); ast (Python 3) has one BinOp/
    # BoolOp/UnaryOp node type with an `.op` attribute holding an operator
    # *instance* (Add(), BitAnd(), Or(), USub(), ...) -- these tuples group
    # the operator classes so `isinstance(expr.op, _ARITH_OPS)` etc. below
    # replicate the old per-class isinstance checks against `expr` itself.
    _ARITH_OPS = (ast.Add, ast.Sub, ast.Mult, ast.Div, ast.Mod, ast.Pow)
    _INT_BINOPS = (ast.FloorDiv, ast.BitAnd, ast.BitOr, ast.BitXor,
    ...
    def _guessTypes(self, expr, curr_ns=None):
        ...
        if isinstance(expr, ast.Constant):
            ...
            ts = [dict.__name__]
        elif isinstance(expr, ast.BinOp) and isinstance(expr.op, self._ARITH_OPS):
            ...
            if not ts and highest > -1:
                ts = [order[highest]]
        elif isinstance(expr, ast.BinOp) and isinstance(expr.op, self._INT_BINOPS):
            ts = [int.__name__]
            ...
            for node in expr.values:
                for t in self._guessTypes(node):
                    if t not in ts:
                        ts.append(t)
        elif isinstance(expr, ast.Compare) or (
            ...
            ts = [type(1==2).__name__]
                ...
                expr.op, (ast.UAdd, ast.USub, ast.Invert, ast.Not)):
            ts = self._guessTypes(expr.operand)
        elif isinstance(expr, ast.Subscript) and isinstance(expr.slice, ast.Slice):
            ts = [list.__name__]
        # No `ast.Backquote` case -- backquote-repr syntax (`` `x` ``) was
        # removed from the language in Python 3, ast has no such node type
        # at all (referencing one would be an AttributeError).
    ...
    def _lambdaArgsRepr(self, a):
        """Shared arg-list repr builder for Lambda/FunctionDef-shaped
        `ast.arguments` objects (used by _getExprRepr's Lambda case)."""
        posargs = list(a.posonlyargs) + list(a.args)
        argnames = [arg.arg for arg in posargs]
    ...
    def _getExprRepr(self, node):
        ...
        if isinstance(node, ast.Name):
            ...
            for kw in node.keywords:
                if kw.arg is None:
                    allargs.append("**" + self._getExprRepr(kw.value))
                else:
                    allargs.append(kw.arg + "=" + self._getExprRepr(kw.value))
            s += ",".join( allargs )
            s += ")"
        elif isinstance(node, ast.Subscript) and isinstance(node.slice, ast.Slice):
            sl = node.slice
            s = self._getExprRepr(node.value)
            s += "["
            if sl.lower:
                s += self._getExprRepr(sl.lower)
            s += ":"
            if sl.upper:
                s += self._getExprRepr(sl.upper)
            s += "]"
        elif isinstance(node, ast.Subscript):
            # Matches the original's minimal (index-omitting) repr for a
            # non-slice subscript, e.g. `a[x]` reprs as just "[]"'s wrapped
            # around the *base* expr, not the index -- preserved as-is
            # rather than "fixed" since callers only use this for a
            # best-effort signature/default-value string.
            s = "[%s]" % self._getExprRepr(node.value)
        # No `ast.Backquote` case -- removed from the language in Python 3.
            ...
            op_repr = op_reprs.get(type(node.op))
            if op_repr is None:
                raise PythonCILEError("don't know how to get string repr "
                                      "of binary operator: %r" % node.op)
            s = self._getExprRepr(node.left) + op_repr + self._getExprRepr(node.right)
        elif isinstance(node, ast.keyword):
            if node.arg is None:
                s = "**" + self._getExprRepr(node.value)
            else:
                s = node.arg + "=" + self._getExprRepr(node.value)
        elif isinstance(node, ast.Lambda):
            s = "lambda"
            args = self._lambdaArgsRepr(node.args)
            if args:
                s += " " + ",".join(args)
            try:
    ...
    def _getCITDLExprRepr(self, node, _level=0):
        ...
        if isinstance(node, ast.Name):
            ...
            # Only allow Call at the top-level. I.e. this:
            #   spam.ham.eggs()
...
def _quietParse(content):
    # ast.parse() doesn't write to stderr the way the old compiler.parse()
    # did, so no stderr-swallowing wrapper is needed anymore -- kept as a
    # thin function anyway so _getAST()'s two call sites read the same.
...
def _getAST(content):
    ...
    """
    # EOL issues: normalize line endings up front (Python 3's ast.parse()
    # handles '\r\n'/'\r' fine on its own, but keeping this normalization
    # avoids relying on that and keeps line numbers predictable for the
    ...
    ast_ = None
    try:
        ast_ = _quietParse(content)
    except SyntaxError as ex:
        errlineno = ex.lineno
        log.debug("ast.parse #1: syntax error on line %d", errlineno)
    ...
    if errlineno is not None:
        ...
        errlineno2 = None
        try:
            ast_ = _quietParse(newContent)
        except SyntaxError as ex:
            errlineno2 = ex.lineno
            log.debug("ast.parse #2: syntax error on line %d", errlineno2)
...
def scan_cix(content, filename, md5sum=None, mtime=None, lang="Python"):
    ...
    conforming the the Code Intelligence XML format.
        ...
        "content" is the Python content to scan. This should be an
            encoded string: must be a string for repr(md5) and
            repr(compiler.parse) -- see bug 73461.
    ...
    # Writing with a real encoding (as opposed to the special "unicode"
    # pseudo-encoding ElementTree also accepts) makes it emit *bytes*,
    # including the leading <?xml ...?> declaration -- Python 2's StringIO
    # tolerated that mixing, Python 3's text-only io.StringIO doesn't, so
    # this uses a real byte stream and decodes back to str afterward.
    ...
    raw_cix = stream.getvalue().decode("utf-8")
...
def scan_et(content, filename, md5sum=None, mtime=None, lang="Python"):
    ...
    if isinstance(content, bytes):
        # buffer.py/accessor.py keep buffer text as raw utf-8 bytes
        # (a carryover from the Python 2 "str is bytes" convention) --
        # everything below assumes a real Python 3 str.
    ...
    # A trailing newline avoids ast.parse() choking on funky *whitespace*
    # at the very end of the file (same reasoning as the original's
    # comment about the old 'compiler' package, ast.parse() has the same
    # quirk).
    content = content.rstrip() + '\n'

    # No more `if lang == "Python3": content = _convert3to2(content)` here
    # -- that regex-based Python-3-to-2 downgrade existed only because the
    # old 'compiler' package couldn't parse genuine Python 3 syntax at
    # all. ast.parse() (used by _getAST() below) handles Python 3 syntax
    # natively regardless of the declared `lang`, so the down-conversion
    # step is gone rather than ported.
    ...
    try:
        ...
        if log.isEnabledFor(logging.INFO):
            ...
            # Route through the logger (pformat), not pprint.pprint() --
            # stdout *is* the OOP wire protocol channel here, and a raw,
            # unframed pprint() write corrupts it for any consumer
            # expecting strict length-prefixed framing (found via a real
            # repro: enabling INFO-level logging while scanning a buffer
            # with a non-empty module namespace silently interleaved a
            # bare `{...}\n` into the byte stream).
            for nspath, namespace in visitor.st.items():
                if len(nspath) == 0: # this is the module namespace
                    log.debug(pprint.pformat(namespace))
