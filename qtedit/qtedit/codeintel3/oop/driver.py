...
import codeintel3.accessor
import codeintel3.buffer
import codeintel3.common
import codeintel3.indexer
import codeintel3.udl
from codeintel3.database.database import Database, DatabaseError
import codeintel3.environment
import collections
import functools
...
class Driver(threading.Thread):
    """
    Out-of-process codeintel3 driver
    ...
    class _DBEventReporter(object):
        def __init__(self, driver):
            ...
            # key is str path, value is number of times it's active
            self._dirs = collections.defaultdict(int)
            # directories which are complete (str path)
        ...
        def onScanStarted(self, description, dirs=set()):
            ...
            @param description {str} A string suitable for showing the user
                about the upcoming operation
            @param dirs {set of str} The directories about to be scanned
            """
        ...
        def onScanDirectory(self, description, dir_path, current=None, total=None):
            ...
            @param dir {str} The directory currently being scanned
        ...
        def onScanComplete(self, dirs=set(), scanned=set()):
            ...
            @param dirs {set of str} The directories that were intially
                   requested to be scanned (as pass in onScanStarted)
            @param scanned {set of str} Directories which were successfully
    ...
    def do_abort(self, request):
        try:
            ...
            log.exception(str(e))
            self.exception(request=request, message=str(e))
    ...
    def do_add_dirs(self, request):
        ...
        codeintel3.udl.UDLLexer.add_extra_lexer_dirs(lexer_dirs)
        ...
        if len(names) > 1:
            ...
            for package_name in names[:-1]:
                spec = importlib.machinery.PathFinder.find_spec(
                    package_name, [path] if path else None
                )
                if not spec:
            ...
            name = names[-1]
        spec = importlib.machinery.PathFinder.find_spec(name, [path] if path else None)
        if not spec:
            raise RequestFailure(msg="load-extension could not find module %r" % (name,))
    ...
    def _read_exact(self, n):
        """Read exactly n characters from fd_in, looping as needed.

        A plain fd_in.read(n) on a Python 3 TextIOWrapper wrapping a pipe
        is only guaranteed to return "at most n" characters, not exactly
        n -- unlike Python 2's file.read(n), which blocked until it got
        the full count (or EOF) for a real pipe fd. Observed empirically:
        a single fd_in.read(2) on a freshly-written 2-char frame can
        return just 1 character, silently truncating the JSON payload
        and breaking every read after it. Loop until satisfied or EOF.
        """
        chunks = []
        remaining = n
        while remaining > 0:
            chunk = self.fd_in.read(remaining)
            if not chunk:
                break
            chunks.append(chunk)
            remaining -= len(chunk)
    ...
    def start(self):
        ...
        while not self.quit:
            ...
            if ch == "{":
                size = int(buf, 10)
                try:
                    buf = ch + self._read_exact(size - 1) # exclude already-read {
                except IOError:
                ...
                try:
                    ...
                    log.exception(e)
                    self.exception(message=str(e), request=None)
                    continue
                finally:
                    buf = ""
...
class CoreHandler(CommandHandler):
    ...
    def do_database_info(self, request, driver):
        ...
        try:
            ...
            if state == Database.UPGRADE_NOT_NECESSARY:
                ...
                driver.send(state="upgrade-blocked", **{"state-detail":details})
        except Exception:
    ...
    def do_scan_document(self, request, driver):
        ...
        if not hasattr(buf, "scan"):
            ...
            return
        priority = request.get("priority", codeintel3.common.PRIORITY_CURRENT)
        ...
        scan_request = codeintel3.indexer.ScanRequest(buf,
                                                      priority,
                                                      mtime=mtime,
                                                      on_complete=on_complete)
        if priority <= codeintel3.common.PRIORITY_IMMEDIATE:
            delay = 0
        else:
            delay = 1.5
    ...
    def do_eval(self, request, driver):
        ...
        try:
            ...
            trg = codeintel3.common.Trigger(**data)
        except AttributeError:
...
class Environment(codeintel3.environment.Environment):
    def __init__(self, request={}, send_fn=None, name=None):
        codeintel3.environment.Environment.__init__(self)
