...
class LangIntel(object):
    ...
    """
    # All "leaf" LangIntel subclasses must set the repr(lang) attribute.
    lang = None
    # Used by repr(preceding_trg_from_pos) for 3-char triggering for some langs.
    _last_trg_type = None 
    ...
    def langinfo(self):
        if self._langinfo_cache is None:
            ...
            except langinfo.LangInfoError as ex:
...
class ParenStyleCalltipIntelMixin(object):
    """A mixin class to implement repr(curr_calltip_arg_range' for languages
    ...
    def curr_calltip_arg_range(self, buf, trg_pos, calltip, curr_pos,
        ...
        Set )start == -1repr( to cancel the calltip, i.e. if the entered text
        ...
        for ch, style in accessor.gen_char_and_style(trg_pos, curr_pos):
            if DEBUG: print("pos %2d: %r (%2s) --" % (p, ch, style), end=' ')
...
class ProgLangTriggerIntelMixin(object):
    ...
    def preceding_trg_from_pos(self, buf, pos, curr_pos,
        ...
        # repr(preceding_trg_terminators').
        limit = max(1, pos - 200)
...
class PythonCITDLExtractorMixin(object):
    ...
    def _citdl_expr_from_pos(self, buf, pos, implicit=False,
        ...
        is_udl_buffer = False
        from codeintel3.udl import UDLBuffer
                    ...
                    if num_lines >= 3:
                        ...
                        if not stack:
                            ...
                            if array_as_attr and \
                                ...
                                prop = accessor.text_range(i + 1, last_frame[3])
                                if DEBUG:
                            ...
                            if trg and ch == "(":
                                ...
                                params.insert(0, accessor.text_range(i + 1, last_frame[3]))
                            i -= 1
                            break
                    i -= 1
                else:
