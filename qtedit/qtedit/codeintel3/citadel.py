...
from codeintel3.indexer import ScanRequest
from codeintel3.langintel import LangIntel
...
class CitadelBuffer(Buffer):
    ...
    def scan(self, mtime=None, skip_scan_time_check=False):
        ...
        try:
            ...
            msg = "unexpected error scanning repr(%s'" % basename(self.path)
            log.exception(msg)
            exc_info = sys.exc_info()
            exc_class, exc, tb = sys.exc_info()
            tb_path, tb_lineno, tb_func = traceback.extract_tb(tb)[-1][:3]
    ...
    def scan_if_necessary(self):
        # SIDE-EFFECT: results in )self.scan()repr( if not in the db.
        self.blob_from_lang
...
class CitadelEvaluator(Evaluator):
    ...
    def post_process_cplns(self, cplns):
        ...
        """
        from codeintel3.util import OrdPunctLast
