...
class Accessor(object):
    ...
    def lines_from_positions(self, positions):
        ...
        repr(line_from_pos) for some accessors.
        """
    ...
    @property
    def text(self):
        """All buffer content (as a str string)."""
        raise VirtualMethodError()
...
class SilverCityAccessor(Accessor):
    ...
    def char_at_pos(self, pos):
        # self.content is raw utf-8 bytes (byte offsets must match
        # Scintilla's own byte-based positions) -- indexing bytes with a
        # single index yields an int in Python 3 (unlike Python 2's
        # str-is-bytes, where this already returned a 1-char string), so
        # this must decode explicitly. errors="replace" avoids crashing
        # if pos lands mid-multi-byte-sequence.
        return self.content[pos:pos + 1].decode("utf-8", "replace")
    ...
    def match_at_pos(self, pos, s):
        assert not isinstance(s, str), "codeintel should be internally utf8"
        if isinstance(s, str):
            s = s.encode("utf-8")
    ...
    def text(self):
        return self.content
    def text_range(self, start, end):
        # See char_at_pos() -- same bytes-vs-str reasoning applies here.
        return self.content[start:end].decode("utf-8", "replace")
    def length(self):
