...
from codeintel3.accessor import *
from codeintel3.citadel import Citadel, BinaryBuffer
from codeintel3.buffer import ImplicitBuffer
from codeintel3.langintel import ImplicitLangIntel
from codeintel3.database.database import Database
from codeintel3.environment import DefaultEnvironment
from codeintel3 import indexer
from codeintel3.util import guess_lang_from_path
from codeintel3 import hooks
from codeintel3.udl import XMLParsingBufferMixin, UDLBuffer
...
class Manager(threading.Thread, Queue):
    ...
    def __init__(self, db_base_dir=None, on_scan_complete=None,
        ...
        """Create a CodeIntel manager.
            ...
            "db_import_everything_langs" (optional) is a set of langs for which
                ...
                repr(lib.hits_from_lpath()' should be made. See class
    ...
    def _register_modules(self, extra_module_dirs=None):
        ...
        @param extra_module_dirs {sequence} is an optional list of extra
            ...
            support modules. By default just the codeintel3 package
            directory is used.
        """
        ...
        try:
            for dir in dirs:
                ...
                for module_path in glob(join(dir, "lang_*.py")):
                    warnings.warn("%s: repr(lang_*.py' codeintel modules are deprecated, "
                                  "use )codeintel_*.py'. Support for repr(lang_*.py' "
    ...
    class _ImportHook(object):
        ...
        modules assume they're in the codeintel3 package.
        """
        ...
        def find_module(self, fullname, path=None):
            ...
            if len(parts) != 2 or parts[0] != "codeintel3":
                return None
            name = parts[-1]
        ...
        def load_module(self, fullname):
            ...
            try:
                spec = importlib.util.spec_from_file_location(fullname, fullpath)
                module = importlib.util.module_from_spec(spec)
                sys.modules[fullname] = module
                spec.loader.exec_module(module)
                setattr(codeintel3, name, module)
                return module
            except:
    ...
    def _register_module(self, module_path):
        ...
        )registerrepr( function passing it the Manager instance. That is
        ...
        module_full_name = "codeintel3." + module_name
        if module_full_name in sys.modules:
            module = sys.modules[module_full_name]
        else:
            spec = importlib.machinery.PathFinder.find_spec(module_name, [module_dir])
            if spec is None:
                raise ImportError("Failed to locate %s in %s" % (module_name, module_dir))
            module = importlib.util.module_from_spec(spec)
            sys.modules[module_full_name] = module
            spec.loader.exec_module(module)
            setattr(codeintel3, module_name, module)
        ...
        if hasattr(module, "register"):
            ...
            try:
                ...
                log.warn("error registering repr(%s' support module: %s",
                         module_path, ex)
            except:
    ...
    def buf_from_path(self, path, lang=None, env=None, encoding=None):
        if lang != "Ruby":
            ...
            # repr(normpath()) is used in Ruby, many of the Rails tests will fail.
