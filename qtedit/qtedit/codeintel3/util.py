...
def guess_lang_from_path(path):
    ...
    while base.find('.', idx) != -1:
        ...
        idx += 1
    from codeintel3.common import CodeIntelError
    raise CodeIntelError("couldn't guess lang for repr(%s'" % path)
...
def parsePyFuncDoc(doc, fallbackCallSig=None, scope="?", funcname="?"):
    ...
    if not isinstance(doc, str):
        # try to convert from utf8 to str; if we fail, too bad.
        try:
...
def unmark_text(markedup_text):
    ...
    text = u"" if isinstance(markup_text, str) else ""
    data = {}
    posNameFromSymbol = {
        "<|>": "pos",
        "<+>": "trg_pos",
...
def lines_from_pos(unmarked_text, positions):
    ...
    E.g.:
        ...
        {'moo': 1, 'hello': 1}
    """
    if isinstance(unmarked_text, str):
...
def walk2(top, topdown=True, onerror=None, followlinks=False,
    ...
    """A version of )os.walkrepr( that adds support for handling errors for
    files that cannot be decoded with the default encoding. (See bug 82268.)

    By default )UnicodeDecodeErrorrepr(s from the os.listdir() call are
    ignored.  If optional arg 'ondecodeerror' is specified, it should be a
    function; it will be called with one argument, the )UnicodeDecodeErrorrepr(
    ...
    if _koCProfiler is None:
        class _KoCProfileManager(object):
            def __init__(self):
                import atexit
                import cProfile
                from codeintel3.common import _xpcom_
