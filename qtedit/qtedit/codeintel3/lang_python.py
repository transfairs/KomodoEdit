...
from codeintel3.common import *
from codeintel3.citadel import (CitadelBuffer, CitadelEvaluator, ImportHandler,
                                CitadelLangIntel)
from codeintel3.indexer import PreloadLibRequest
from codeintel3 import pythoncile
from codeintel3.util import (banner, indent, markup_text, isident, isdigit,
                             makePerformantLogger)
from codeintel3 import tree
from codeintel3.tree_python import PythonTreeEvaluator, PythonImportLibGenerator
from codeintel3.langintel import (ParenStyleCalltipIntelMixin,
                                  ProgLangTriggerIntelMixin,
                                  PythonCITDLExtractorMixin)

from codeintel3.tree import tree_from_cix
...
class PythonLangIntel(CitadelLangIntel, ParenStyleCalltipIntelMixin,
    ...
    def interpreter_from_env(self, env):
        ...
        if not python or not exists(python):
            ...
            if self.lang == "Python":
                # codeintel3 only registers "Python" (lang_python3.py, the
                # separate Python-3-as-target-source analyzer, is out of
                # scope for this port -- see modernization_roadmap). Since
                # qtedit is used to edit real Python 3 code and modern
                # systems typically have no "python"/"python2" binary at
                # all, prefer "python3" first while still tolerating an
                # old-style install.
                exe_names = ["python3", "python", "python2"]
                acceptable_major_versions = (3, 2)
            elif self.lang == "Python3":
                # For Python 3, prefer "python3" over "python"
                exe_names = ["python3", "python"]
                acceptable_major_versions = (3,)
            candidates = []
            ...
            for python in candidates:
                try:
                    ...
                    if int(version.split(".")[0]) in acceptable_major_versions:
                        break
                except:
                    pass
        ...
        if libs is None:
            ...
            for dir in sys_path:
                ...
                if dir == "": # -> curdirlib (already handled)
                    ...
                    #log.warn("repr(%s': not handling .egg file on Python sys.path",
                    #         dir)
                    continue
...
class PythonImportHandler(ImportHandler):
    ...
    def _findScannableFiles(self, walk_arg, dirname, names):
        # Dead code even in the original Python-2 codeintel3 tree (no call
        # site anywhere, a leftover from the pre-os.walk os.path.walk
        # callback convention that no longer exists in Python 3 either) --
        # only kept parseable, not verified to still work.
        files, searchedDirs, skipRareImports, importableOnly = walk_arg
...
class PythonCILEDriver(CILEDriver):
    ...
    def scan_binary(self, buf):
        from codeintel3 import pybinary
