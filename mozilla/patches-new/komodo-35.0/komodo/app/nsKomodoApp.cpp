...
static void InitKomodoRuntimeEnv()
{
  // Avoid inheriting host GTK/GIO schema/module overrides into Komodo.
  unsetenv("GTK_PATH");
  ...
  // Keep Linux startup behavior stable while still allowing explicit overrides.
  SetEnvDefault("GDK_BACKEND", "x11");
  SetEnvDefault("MOZ_ENABLE_WAYLAND", "0");
  SetEnvDefault("KOMODO_SKIP_FCFONTSETSORT", "1");
  SetEnvDefault("KOMODO_FORCE_TOPLEVEL_MAP", "1");
  SetEnvDefault("KOMODO_DISABLE_CACHE2", "1");
  SetEnvDefault("KOMODO_DISABLE_SHUTDOWN_TERMINATOR", "1");
}
#endif
...
static int do_main(const char *xpcomDllPath, int argc, char* argv[])
{
  ...
  const char* bypassKoStart = getenv("KOMODO_BYPASS_KOSTART");
  if (bypassKoStart && bypassKoStart[0] && bypassKoStart[0] != '0') {
    return XRE_main(argc, argv, &appData, 0);
  }
